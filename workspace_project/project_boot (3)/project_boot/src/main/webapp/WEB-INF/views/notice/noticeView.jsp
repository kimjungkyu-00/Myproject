			...
			<c:forEach items="${commentList }" var="commentVO" varStatus="status">
				<tr data-commentidx=${commentVO.idx}>
					...
					<td><input type="text" name="contents"
						...
						onclick="commentUpdateview(${commentVO.idx});"> <input
						type="button" value="삭제" readonly="readonly"
						onclick="commentDelete(${commentVO.idx});"></td>
				</tr>
				   </c:if>
			</c:forEach>
		</tbody>
...
-->
	...
	<form action="">
		<input type="text" name="contents" id="contents" /> <input
			type="button" class="btn btn-outline btn-primary" value="댓글작성"
			onclick="addComment();" />
	</form>
